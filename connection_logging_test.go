...
func TestConnectionLoggingCryptoFrame(t *testing.T) {
	...
	require.Equal(t, &qlogevents.CryptoFrame{
		Offset: 1234,
		Length: 6,
	}, f.Frame)
}
...
func TestConnectionLoggingStreamFrame(t *testing.T) {
	...
	require.Equal(t, &qlogevents.StreamFrame{
		StreamID: 42,
		Offset:   1234,
		Length:   3,
		Fin:      true,
...
func TestConnectionLoggingAckFrame(t *testing.T) {
	...
	require.Equal(t, &qlogevents.AckFrame{
		AckRanges: []wire.AckRange{
...
func TestConnectionLoggingDatagramFrame(t *testing.T) {
	...
	require.Equal(t, &qlogevents.DatagramFrame{Length: 6}, f.Frame)
}

func TestConnectionLoggingOtherFrames(t *testing.T) {
	f := toQlogFrame(&wire.MaxDataFrame{MaximumData: 1234})
	require.Equal(t, &qlogevents.MaxDataFrame{MaximumData: 1234}, f.Frame)
}
