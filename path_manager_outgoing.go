...
// After abandoning, it is possible to probe this path again by calling Probe.
func (p *Path) Abandon() error {
	close(p.abandon)
	return p.pathManager.removePath(p.id)
}
...
func (pm *pathManagerOutgoing) addPath(p *Path, done chan<- struct{}) {
	var b [8]byte
	_, _ = rand.Read(b[:])
	pm.mx.Lock()
...
func (pm *pathManagerOutgoing) NewPath(t *Transport) *Path {
	pm.mx.Lock()
	defer pm.mx.Unlock()
