...
var _ = Describe("Public Header", func() {
	Context("when parsing", func() {
		It("accepts a sample client header", func() {
			...
			hdr, err := Parse(b, protocol.PerspectiveClient)
		...
		It("does not accept 0-byte connection ID", func() {
			...
			_, err := Parse(b, protocol.PerspectiveClient)
		...
		It("reads a PublicReset packet", func() {
			...
			hdr, err := Parse(b, protocol.PerspectiveServer)
			Expect(err).ToNot(HaveOccurred())
		...
		Context("version negotiation packets", func() {
			...
			It("errors on invalid version tags", func() {
				...
				b := bytes.NewReader(data)
				_, err := Parse(b, protocol.PerspectiveServer)
