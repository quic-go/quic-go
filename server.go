...
func (s *baseServer) handlePacket(p receivedPacket) {
	...
	default:
		...
		if s.qlogger != nil {
			s.qlogger.RecordEvent(qlogevents.PacketDropped{
...
func (s *baseServer) handlePacketImpl(p receivedPacket) bool /* is the buffer still in use? */ {
	...
	if !protocol.IsSupportedVersion(s.config.Versions, v) {
		if s.disableVersionNegotiation {
			if s.qlogger != nil {
				s.qlogger.RecordEvent(qlogevents.PacketDropped{
					Header:  qlogevents.PacketHeader{Version: v},
					Raw:     qlogevents.RawInfo{Length: int(p.Size())},
	...
	if hdr.Type != protocol.PacketTypeInitial {
		...
		if s.qlogger != nil {
			var pt qlogevents.PacketType
			switch hdr.Type {
			case protocol.PacketTypeInitial:
				pt = qlogevents.PacketTypeInitial
			case protocol.PacketTypeHandshake:
				pt = qlogevents.PacketTypeHandshake
			case protocol.PacketType0RTT:
				pt = qlogevents.PacketType0RTT
			}
			s.qlogger.RecordEvent(qlogevents.PacketDropped{
				Header: qlogevents.PacketHeader{
					PacketType:   pt,
...
func (s *baseServer) handleInitialImpl(p receivedPacket, hdr *wire.Header) error {
	...
	var qlogger qlog.Recorder
	if config.QlogTrace != nil {
		// Use the same connection ID that is passed to the client's GetLogWriter callback.
		connID := hdr.DestConnectionID
		if origDestConnID.Len() > 0 {
			connID = origDestConnID
		}
		qlogTrace := config.QlogTrace(ctx, false, connID)
...
func (s *baseServer) sendRetryPacket(p rejectedPacket) error {
	...
	if s.qlogger != nil {
		s.qlogger.RecordEvent(qlogevents.PacketSent{
			Header: qlogevents.PacketHeader{
				...
				Token:            &qlogevents.Token{Raw: token},
			},
			Raw: qlogevents.RawInfo{
				Length:        int(len(buf.Data)),
...
func (s *baseServer) sendError(remoteAddr net.Addr, hdr *wire.Header, sealer handshake.LongHeaderSealer, errorCode qerr.TransportErrorCode, info packetInfo) error {
	...
	if s.qlogger != nil {
		s.qlogger.RecordEvent(qlogevents.PacketSent{
			...
			Raw: qlogevents.RawInfo{
				Length:        int(len(b.Data)),
				PayloadLength: int(replyHdr.Length),
			},
			Frames: []qlogevents.Frame{{Frame: ccf}},
		})
	}
...
func (s *baseServer) maybeSendVersionNegotiationPacket(p receivedPacket) {
	...
	if s.qlogger != nil {
		s.qlogger.RecordEvent(qlogevents.VersionNegotiationSent{
