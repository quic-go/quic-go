...
var _ = Describe("Send Stream", func() {
	...
	Context("writing", func() {
		It("writes and gets all data at once", func() {
			...
			go func() {
				...
				mockSender.EXPECT().onStreamDataWrittenByApplication(gomock.Any(), protocol.ByteCount(0), protocol.ByteCount(6))
		...
		It("bundles small writes", func() {
			...
			go func() {
				...
				mockSender.EXPECT().onStreamDataWrittenByApplication(gomock.Any(), protocol.ByteCount(0), protocol.ByteCount(3))
				mockSender.EXPECT().onStreamDataWrittenByApplication(gomock.Any(), protocol.ByteCount(3), protocol.ByteCount(3))
		...
		It("writes and gets data in multiple turns, for large writes", func() {
			...
			go func() {
				defer GinkgoRecover()
				mockSender.EXPECT().onHasStreamData(streamID)
				mockSender.EXPECT().onStreamDataWrittenByApplication(gomock.Any(), protocol.ByteCount(0), protocol.ByteCount(5000))
		...
		It("says if it has more data for writing", func() {
			...
			go func() {
				...
				defer close(done)
				mockSender.EXPECT().onHasStreamData(streamID)
				mockSender.EXPECT().onStreamDataWrittenByApplication(gomock.Any(), gomock.Any(), protocol.ByteCount(100))
		...
		It("copies the slice while writing", func() {
			...
			go func() {
				...
				defer close(done)
				mockSender.EXPECT().onHasStreamData(streamID)
				mockSender.EXPECT().onStreamDataWrittenByApplication(gomock.Any(), gomock.Any(), protocol.ByteCount(3))
		...
		Context("deadlines", func() {
			...
			It("returns the number of bytes written, when the deadline expires", func() {
				...
				go func() {
					...
					mockSender.EXPECT().onHasStreamData(streamID)
					mockSender.EXPECT().onStreamDataWrittenByApplication(gomock.Any(), gomock.Any(), gomock.Not(protocol.ByteCount(5000)))
					var err error
					n, err = strWithTimeout.Write(getData(5000))
					Expect(err).To(MatchError(errDeadline))
					Expect(time.Now()).To(BeTemporally("~", deadline, scaleDuration(20*time.Millisecond)))
	...
	Context("determining when a stream is completed", func() {
		...
		It("retransmits data until everything has been acknowledged", func() {
			...
			mockSender.EXPECT().onStreamDataWrittenByApplication(gomock.Any(), gomock.Any(), protocol.ByteCount(dataLen))
