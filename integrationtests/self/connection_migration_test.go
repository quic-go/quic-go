...
func TestConnectionMigration(t *testing.T) {
	...
	tr1 := &quic.Transport{Conn: newUPDConnLocalhostOnPort(t, 10000)}
	defer tr1.Close()
	tr2 := &quic.Transport{Conn: newUPDConnLocalhostOnPort(t, 20000)}
	defer tr2.Close()
	...
	conn, err := tr1.Dial(ctx, proxy.LocalAddr(), tlsConf, getQuicConfig(nil))
	require.NoError(t, err)
