...
func TestHTTPServerRequestParsing(t *testing.T) {
	...
	// GET, HEAD and POST cover methods that use origin form. OPTIONS * and
	// CONNECT use distinct request-target forms. The escaped path covers URL
	// parsing beyond the common case.
	tests := []struct {
		name      string
		method    string
		authority string
