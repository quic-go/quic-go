...
var _ = Describe("TLS session resumption", func() {
	It("uses session resumption", func() {
		...
		Expect(sess.ConnectionState().TLSConnectionState.DidResume).To(BeFalse())

		serverSess, err := server.Accept(context.Background())
		Expect(err).ToNot(HaveOccurred())
		Expect(serverSess.ConnectionState().TLSConnectionState.DidResume).To(BeFalse())
		...
		Expect(sess.ConnectionState().TLSConnectionState.DidResume).To(BeTrue())

		serverSess, err = server.Accept(context.Background())
		Expect(err).ToNot(HaveOccurred())
		Expect(serverSess.ConnectionState().TLSConnectionState.DidResume).To(BeTrue())
	})
