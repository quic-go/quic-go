...
func newCryptoSetup(
	...
	if qlogger != nil {
		qlogger.RecordEvent(qlogevents.KeyUpdated{
			Trigger: qlogevents.KeyUpdateTLS,
...
func (h *cryptoSetup) ChangeConnectionID(id protocol.ConnectionID) {
	...
	if h.qlogger != nil {
		h.qlogger.RecordEvent(qlogevents.KeyUpdated{
			Trigger: qlogevents.KeyUpdateTLS,
			KeyType: encLevelToKeyType(protocol.EncryptionInitial, protocol.PerspectiveClient),
		})
		h.qlogger.RecordEvent(qlogevents.KeyUpdated{
...
func (h *cryptoSetup) setWriteKey(el tls.QUICEncryptionLevel, suiteID uint16, trafficSecret []byte) {
	...
	case tls.QUICEncryptionLevelApplication:
		...
		if h.zeroRTTSealer != nil {
			...
			if h.qlogger != nil {
				h.qlogger.RecordEvent(qlogevents.KeyDiscarded{KeyType: qlogevents.KeyTypeClient0RTT})
			}
		}
	default:
...
func (h *cryptoSetup) DiscardInitialKeys() {
	...
	if dropped {
		...
		if h.qlogger != nil {
			h.qlogger.RecordEvent(qlogevents.KeyDiscarded{KeyType: qlogevents.KeyTypeClientInitial})
			h.qlogger.RecordEvent(qlogevents.KeyDiscarded{KeyType: qlogevents.KeyTypeServerInitial})
		}
	}
}
...
func (h *cryptoSetup) SetHandshakeConfirmed() {
	...
	if dropped {
		...
		if h.qlogger != nil {
			h.qlogger.RecordEvent(qlogevents.KeyDiscarded{KeyType: qlogevents.KeyTypeClientHandshake})
			h.qlogger.RecordEvent(qlogevents.KeyDiscarded{KeyType: qlogevents.KeyTypeServerHandshake})
		}
	}
}
...
func encLevelToKeyType(encLevel protocol.EncryptionLevel, pers protocol.Perspective) qlogevents.KeyType {
	if pers == protocol.PerspectiveServer {
		switch encLevel {
		case protocol.EncryptionInitial:
			return qlogevents.KeyTypeServerInitial
		case protocol.EncryptionHandshake:
			return qlogevents.KeyTypeServerHandshake
		case protocol.Encryption0RTT:
			return qlogevents.KeyTypeServer0RTT
		case protocol.Encryption1RTT:
			return qlogevents.KeyTypeServer1RTT
		default:
			return ""
		}
	}
	switch encLevel {
	case protocol.EncryptionInitial:
		return qlogevents.KeyTypeClientInitial
	case protocol.EncryptionHandshake:
		return qlogevents.KeyTypeClientHandshake
	case protocol.Encryption0RTT:
		return qlogevents.KeyTypeClient0RTT
	case protocol.Encryption1RTT:
		return qlogevents.KeyTypeClient1RTT
	default:
		return ""
	}
}
