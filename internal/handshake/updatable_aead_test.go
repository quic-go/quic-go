...
func bothSides(ev qlog.Event) []qlog.Event {
	switch ev := ev.(type) {
	case qlogevents.KeyDiscarded:
		return []qlog.Event{
			qlogevents.KeyDiscarded{
				KeyType:  qlogevents.KeyTypeClient1RTT,
				KeyPhase: ev.KeyPhase,
			},
			qlogevents.KeyDiscarded{
				KeyType:  qlogevents.KeyTypeServer1RTT,
				KeyPhase: ev.KeyPhase,
			},
		}
	case qlogevents.KeyUpdated:
		return []qlog.Event{
			qlogevents.KeyUpdated{
				KeyType:  qlogevents.KeyTypeClient1RTT,
				KeyPhase: ev.KeyPhase,
				Trigger:  ev.Trigger,
			},
...
func TestReorderedPacketAfterKeyUpdate(t *testing.T) {
	...
	require.Equal(t, protocol.KeyPhaseOne, server.KeyPhase())
	require.Equal(t,
		bothSides(qlogevents.KeyUpdated{Trigger: qlogevents.KeyUpdateRemote, KeyPhase: 1}),
...
func TestDropsKeys3PTOsAfterKeyUpdate(t *testing.T) {
	...
	require.Equal(t, protocol.KeyPhaseOne, server.KeyPhase())
	require.Equal(t,
		bothSides(qlogevents.KeyUpdated{KeyPhase: 1, Trigger: qlogevents.KeyUpdateRemote}),
		eventRecorder.Events(),
	)
	eventRecorder.Clear()
	...
	require.Equal(t,
		bothSides(qlogevents.KeyDiscarded{KeyPhase: 0}),
		eventRecorder.Events(),
	)
}
...
func TestInitiateKeyUpdateAfterSendingMaxPackets(t *testing.T) {
	...
	require.Equal(t, protocol.KeyPhaseOne, server.KeyPhase())
	require.Equal(t,
		bothSides(qlogevents.KeyUpdated{KeyPhase: 1, Trigger: qlogevents.KeyUpdateLocal}),
		eventRecorder.Events(),
	)
	eventRecorder.Clear()
	...
	require.Equal(t,
		append(
			bothSides(qlogevents.KeyDiscarded{KeyPhase: 0}),
			bothSides(qlogevents.KeyUpdated{KeyPhase: 2, Trigger: qlogevents.KeyUpdateLocal})...,
		),
		eventRecorder.Events(),
	)
}
...
func TestFastKeyUpdatesByPeer(t *testing.T) {
	...
	require.Equal(t,
		append(
			bothSides(qlogevents.KeyDiscarded{KeyPhase: 0}),
			bothSides(qlogevents.KeyUpdated{KeyPhase: 2, Trigger: qlogevents.KeyUpdateRemote})...,
		),
		eventRecorder.Events(),
	)
}
