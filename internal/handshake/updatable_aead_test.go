...
func TestKeyUpdateEnforceACKInKeyPhase(t *testing.T) {
	const firstKeyUpdateInterval = 5
	setKeyUpdateIntervals(t, firstKeyUpdateInterval, KeyUpdateInterval)
...
func TestInitiatesKeyUpdatesAfterOpeningMaxPackets(t *testing.T) {
	const firstKeyUpdateInterval = 5
	const keyUpdateInterval = 20
	setKeyUpdateIntervals(t, firstKeyUpdateInterval, keyUpdateInterval)
...
// 		dataKeyPhaseZero := client.Seal(nil, []byte(msg), 1, []byte(ad))
// 		_, err = server.Open(nil, dataKeyPhaseZero, now.Add(threePTO).Add(time.Second), 1, protocol.KeyPhaseZero, []byte(ad))
// 		Expect(err).ToNot(HaveOccurred())
// 		// Now receive a packet with key phase 1.
// 		// This should start the timer to drop the keys after 3 PTOs.
// 		client.rollKeys()
// 		dataKeyPhaseOne := client.Seal(nil, []byte(msg), 10, []byte(ad))
// 		t := now.Add(threePTO).Add(time.Second)
// 		serverTracer.EXPECT().UpdatedKey(protocol.KeyPhase(1), true)
// 		_, err = server.Open(nil, dataKeyPhaseOne, t, 10, protocol.KeyPhaseOne, []byte(ad))
// 		Expect(err).ToNot(HaveOccurred())
// 		// Make sure the keys are still here.
// 		_, err = server.Open(nil, dataKeyPhaseZero, t.Add(threePTO*9/10), 1, protocol.KeyPhaseZero, []byte(ad))
// 		Expect(err).ToNot(HaveOccurred())
// 		serverTracer.EXPECT().DroppedKey(protocol.KeyPhase(0))
// 		_, err = server.Open(nil, dataKeyPhaseZero, t.Add(threePTO).Add(time.Nanosecond), 1, protocol.KeyPhaseZero, []byte(ad))
