...
func (hs *serverHandshakeStateTLS13) processClientHello() error {
	...
	// Handle pure PQC key exchanges
	if selectedGroup == MLKEM768 || selectedGroup == MLKEM1024 {
		pqcKey, err := processPQCClientKeyShare(selectedGroup, clientKeyShare.data, c.config.rand())
		if err != nil {
			c.sendAlert(alertIllegalParameter)
			return fmt.Errorf("tls: invalid PQC client key share: %w", err)
		}
		hs.hello.serverShare = keyShare{group: selectedGroup, data: pqcKey.ciphertext}
		hs.sharedKey = pqcKey.sharedSecret
	} else {
