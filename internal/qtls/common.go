...
// fipsRequired is a stub for FIPS mode checking
// We don't support FIPS mode in qtls fork
func fipsRequired() bool {
	return false
}

// godebug stub for compatibility - all flags disabled
type godebugSetting struct {
	name string
}

func newGodebug(name string) *godebugSetting {
...
func (g *godebugSetting) IncNonDefault() {
	// No-op
}
...
const (
	...
	// Custom PQC curve IDs (using private range 0xFF00-0xFFFF)
	MLKEM768  CurveID = 0xFF01 // Pure ML-KEM-768 (192-bit security)
	MLKEM1024 CurveID = 0xFF02 // Pure ML-KEM-1024 (256-bit security)
)

func isTLS13OnlyKeyExchange(curve CurveID) bool {
	return curve == X25519MLKEM768 || curve == MLKEM768 || curve == MLKEM1024
}
...
var tls10server = newGodebug("tls10server")
...
func (c *Config) supportedVersions(isClient bool) []uint16 {
	...
	for _, v := range supportedVersions {
		if fipsRequired() && !slices.Contains(allowedSupportedVersionsFIPS, v) {
			continue
		}
...
func (c *Config) curvePreferences(version uint16) []CurveID {
	...
	if c != nil && len(c.CurvePreferences) != 0 {
		// Use explicitly set preferences
		curvePreferences = c.CurvePreferences
	} else {
		// Use defaults
...
// SetCipherSuite is a stub for compatibility with fuzzing/testing code.
// It returns a no-op function since we don't support forcing cipher suites.
func SetCipherSuite(id uint16) func() {
	return func() {}
}
...
var tlssha1 = newGodebug("tlssha1")

func isDisabledSignatureAlgorithm(version uint16, s SignatureScheme, isCert bool) bool {
	if fipsRequired() && !slices.Contains(allowedSignatureAlgorithmsFIPS, s) {
		return true
	}
