...
func (hs *clientHandshakeStateTLS13) handshake() error {
	...
	if hs.keyShareKeys == nil || (hs.keyShareKeys.ecdhe == nil && hs.keyShareKeys.pqc == nil) || len(hs.hello.keyShares) == 0 {
...
func (hs *clientHandshakeStateTLS13) establishHandshakeKeys() error {
	...
	// Handle pure PQC key exchange (MLKEM768, MLKEM1024)
	if isPurePQCCurve(hs.serverHello.serverShare.group) {
		if hs.keyShareKeys.pqc == nil {
			return c.sendAlert(alertInternalError)
		}
		peerPublicKey := hs.serverHello.serverShare.data
		sharedKey, err = hs.keyShareKeys.pqc.DeriveSharedSecret(peerPublicKey, true)
		if err != nil {
			c.sendAlert(alertIllegalParameter)
			return errors.New("tls: invalid PQC server key share")
		}
	} else {
		// Handle ECDH or hybrid key exchange
