...
func (c *Conn) makeClientHello() (*clientHelloMsg, *keySharePrivateKeys, *echClientContext, error) {
	...
	if maxVersion >= VersionTLS13 {
		...
		} else if curveID == MLKEM768 || curveID == MLKEM1024 {
			// Pure PQC key exchange
			keyShareKeys.pqc, err = generatePQCKeyShare(config.rand(), curveID)
			if err != nil {
				return nil, nil, nil, err
			}
			hello.keyShares = []keyShare{{group: curveID, data: keyShareKeys.pqc.PublicKey()}}
			// Also send X25519 as fallback if it's in the supported curves
			if slices.Contains(hello.supportedCurves, X25519) {
				keyShareKeys.ecdhe, err = generateECDHEKey(config.rand(), X25519)
				if err != nil {
					return nil, nil, nil, err
				}
				hello.keyShares = append(hello.keyShares, keyShare{group: X25519, data: keyShareKeys.ecdhe.PublicKey().Bytes()})
			}
		} else {
...
func (c *Conn) loadSession(hello *clientHelloMsg) (
	...
	if session.version != VersionTLS13 {
		...
		if !session.extMasterSecret && fipsRequired() {
			return nil, nil, nil, nil
		}
...
func (hs *clientHandshakeState) pickCipherSuite() error {
	...
	if hs.c.config.CipherSuites == nil && !fipsRequired() && rsaKexCiphers[hs.suite.id] {
		tlsrsakex.Value() // ensure godebug is initialized
		tlsrsakex.IncNonDefault()
	}
	if hs.c.config.CipherSuites == nil && !fipsRequired() && tdesCiphers[hs.suite.id] {
...
var tlsmaxrsasize = newGodebug("tlsmaxrsasize")
