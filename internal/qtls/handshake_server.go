...
func (hs *serverHandshakeState) pickCipherSuite() error {
	...
	if c.config.CipherSuites == nil && !fipsRequired() && rsaKexCiphers[hs.suite.id] {
		tlsrsakex.Value() // ensure godebug is initialized
		tlsrsakex.IncNonDefault()
	}
	if c.config.CipherSuites == nil && !fipsRequired() && tdesCiphers[hs.suite.id] {
...
func (hs *serverHandshakeState) checkForResumption() error {
	...
	if !sessionState.extMasterSecret && fipsRequired() {
...
func (c *Conn) processCertsFromClient(certificate Certificate) error {
	...
	for i, asn1Data := range certificates {
		// Try parsing as PQC/hybrid certificate first, fall back to standard parsing
		if IsHybridCertificateBytes(asn1Data) {
			_, certs[i], err = ParseHybridCertificate(asn1Data)
		} else if IsMLDSACertificateBytes(asn1Data) {
