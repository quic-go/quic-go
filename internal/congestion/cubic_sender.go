...
type cubicSender struct {
	...
	lastState qlogevents.CongestionState
...
func newCubicSender(
	...
	if c.qlogger != nil {
		c.lastState = qlogevents.CongestionStateSlowStart
		c.qlogger.RecordEvent(qlogevents.CongestionStateUpdated{
			State: qlogevents.CongestionStateSlowStart,
		})
	}
	return c
}
...
func (c *cubicSender) MaybeExitSlowStart() {
	if c.InSlowStart() &&
		...
		c.maybeQlogStateChange(qlogevents.CongestionStateCongestionAvoidance)
	}
}
...
func (c *cubicSender) OnCongestionEvent(packetNumber protocol.PacketNumber, lostBytes, priorInFlight protocol.ByteCount) {
	...
	c.maybeQlogStateChange(qlogevents.CongestionStateRecovery)
...
func (c *cubicSender) maybeIncreaseCwnd(
	...
	if !c.isCwndLimited(priorInFlight) {
		...
		c.maybeQlogStateChange(qlogevents.CongestionStateApplicationLimited)
		return
	}
	...
	if c.InSlowStart() {
		...
		c.maybeQlogStateChange(qlogevents.CongestionStateSlowStart)
		return
	}
...
func (c *cubicSender) maybeQlogStateChange(new qlogevents.CongestionState) {
	if c.qlogger == nil || new == c.lastState {
		return
	}
	c.qlogger.RecordEvent(qlogevents.CongestionStateUpdated{State: new})
	c.lastState = new
}
