...
type alarmTimer struct {
	Time            monotime.Time
	TimerType       qlogevents.TimerType
...
type sentPacketHandler struct {
	...
	lastMetrics qlogevents.MetricsUpdated
	logger      utils.Logger
}
...
func (h *sentPacketHandler) DropPackets(encLevel protocol.EncryptionLevel, now monotime.Time) {
	...
	if h.qlogger != nil && h.ptoCount != 0 {
		h.qlogger.RecordEvent(qlogevents.MetricsUpdated{PTOCount: pointer(uint32(0))})
	}
	h.ptoCount = 0
	h.numProbesToSend = 0
	h.ptoMode = SendNone
...
func (h *sentPacketHandler) qlogMetricsUpdated() {
	var metricsUpdatedEvent qlogevents.MetricsUpdated
	var updated bool
...
func (h *sentPacketHandler) detectSpuriousLosses(ack *wire.AckFrame, ackTime monotime.Time) {
	...
	for pn, sendTime := range h.lostPackets.All() {
		...
		if pn <= ackRange.Largest {
			...
			if h.qlogger != nil {
				h.qlogger.RecordEvent(qlogevents.SpuriousLoss{
...
func (h *sentPacketHandler) setLossDetectionTimer(now monotime.Time) {
	...
	if !hasAlarm && !oldAlarm.Time.IsZero() {
		...
		if h.qlogger != nil {
			h.qlogger.RecordEvent(qlogevents.LossTimerUpdated{
				Type: qlogevents.LossTimerUpdateTypeCancelled,
			})
		}
	}
	...
	if h.qlogger != nil && hasAlarm && newAlarm != oldAlarm {
		h.qlogger.RecordEvent(qlogevents.LossTimerUpdated{
			Type:      qlogevents.LossTimerUpdateTypeSet,
...
func (h *sentPacketHandler) lossDetectionTime(now monotime.Time) alarmTimer {
	...
	if !lossTime.IsZero() && (pathProbeLossTime.IsZero() || lossTime.Before(pathProbeLossTime)) {
		return alarmTimer{
			Time:            lossTime,
			TimerType:       qlogevents.TimerTypeACK,
	...
	if !ptoTime.IsZero() && (pathProbeLossTime.IsZero() || ptoTime.Before(pathProbeLossTime)) {
		return alarmTimer{
			Time:            ptoTime,
			TimerType:       qlogevents.TimerTypePTO,
			EncryptionLevel: encLevel,
		}
	}
	if !pathProbeLossTime.IsZero() {
		return alarmTimer{
			...
			TimerType:       qlogevents.TimerTypePathProbe,
			EncryptionLevel: protocol.Encryption1RTT,
		}
	}
	return alarmTimer{}
	...
	for pn, p := range pnSpace.history.Packets() {
		...
		if !p.SendTime.After(lostSendTime) {
			...
			if !p.isPathProbePacket {
				...
				if h.qlogger != nil {
					h.qlogger.RecordEvent(qlogevents.PacketLost{
						Header: qlogevents.PacketHeader{
							PacketType:   qlogevents.EncryptionLevelToPacketType(p.EncryptionLevel),
							PacketNumber: pn,
						},
						Trigger: qlogevents.PacketLossTimeThreshold,
					})
				}
			}
			...
			if !p.isPathProbePacket {
				...
				if h.qlogger != nil {
					h.qlogger.RecordEvent(qlogevents.PacketLost{
						...
						Trigger: qlogevents.PacketLossReorderingThreshold,
					})
				}
			}
...
func (h *sentPacketHandler) OnLossDetectionTimeout(now monotime.Time) error {
	...
	if !earliestLossTime.IsZero() {
		...
		if h.qlogger != nil {
			h.qlogger.RecordEvent(qlogevents.LossTimerUpdated{
				Type:      qlogevents.LossTimerUpdateTypeExpired,
				TimerType: qlogevents.TimerTypeACK,
				EncLevel:  encLevel,
			})
		}
	...
	if h.qlogger != nil {
		...
		h.qlogger.RecordEvent(qlogevents.MetricsUpdated{PTOCount: pointer(h.ptoCount)})
	}
	h.numProbesToSend += 2
