...
func testSentPacketHandlerPTO(t *testing.T, encLevel protocol.EncryptionLevel, ptoMode SendMode) {
	...
	require.Equal(t,
		...
		eventRecorder.Events(qlogevents.MetricsUpdated{}, qlogevents.LossTimerUpdated{}),
	)
	...
	require.Equal(t,
		...
		eventRecorder.Events(qlogevents.LossTimerUpdated{}, qlogevents.MetricsUpdated{}),
	)
	eventRecorder.Clear()
	// PTO timer expiration doesn't declare packets lost
	require.Empty(t, packets.Lost)
	...
	require.Len(t, eventRecorder.Events(qlogevents.PacketLost{}), 2)
	require.Equal(t,
		[]qlog.Event{
			qlogevents.MetricsUpdated{PTOCount: pointer(uint32(0))},
		},
		eventRecorder.Events(qlogevents.MetricsUpdated{})[:1],
	)
	require.Equal(t,
		[]qlog.Event{
			qlogevents.LossTimerUpdated{
...
func TestSentPacketHandlerSpuriousLoss(t *testing.T) {
	...
	require.Equal(t,
		...
		eventRecorder.Events(qlogevents.SpuriousLoss{}),
	)
	eventRecorder.Clear()
