...
var _ = Describe("Server", func() {
	...
	It("fills in default values if options are not set in the Config", func() {
		...
		Expect(server.config.AttackTimeout).To(Equal(protocol.DefaultAttackTimeout))
	...
	It("setups with the right values", func() {
		...
		Expect(server.config.AttackTimeout).To(Equal(50 * time.Second))
