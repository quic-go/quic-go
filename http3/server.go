	...
	for {
		...
		go func() {
			rerr := s.handleRequest(conn, str, decoder, func() {
				conn.CloseWithError(quic.ApplicationErrorCode(ErrFrameUnexpected), "")
			})
	...
	for {
		...
		go func(str quic.ReceiveStream) {
			...
			case streamTypePushStream: // only the server can push
				conn.CloseWithError(quic.ApplicationErrorCode(ErrStreamCreationError), "")
				return
			default:
...
func (s *Server) handleRequest(conn quic.Connection, str quic.Stream, decoder *qpack.Decoder, onFrameError func()) requestError {
	...
	if err != nil {
		...
		return newStreamError(ErrRequestIncomplete, err)
	}
	hf, ok := frame.(*headersFrame)
	if !ok {
		return newConnError(ErrFrameUnexpected, errors.New("expected first frame to be a HEADERS frame"))
	}
	if hf.Length > s.maxHeaderBytes() {
		return newStreamError(ErrFrameError, fmt.Errorf("HEADERS frame too large: %d bytes (max: %d)", hf.Length, s.maxHeaderBytes()))
	}
	...
	if err != nil {
		...
		return newConnError(ErrGeneralProtocolError, err)
	}
	req, err := requestFromHeaders(hfs)
	if err != nil {
		// TODO: use the right error code
		return newStreamError(ErrGeneralProtocolError, err)
	}
