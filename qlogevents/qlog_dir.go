...
func DefaultConnectionTracer(_ context.Context, isClient bool, connID ConnectionID) qlog.Trace {
	...
	fileSeq := qlog.NewConnectionFileSeq(utils.NewBufferedWriteCloser(bufio.NewWriter(f), f), isClient, connID)
	go fileSeq.Run()
	return fileSeq
}
