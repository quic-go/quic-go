...
var _ = Describe("Retransmission queue", func() {
	...
	Context("Initial data", func() {
		...
		It("adds a PING", func() {
			...
			Expect(q.HasData(protocol.EncryptionInitial)).To(BeTrue())
			Expect(q.GetFrame(protocol.EncryptionInitial, protocol.MaxByteCount, protocol.Version1)).To(Equal(&wire.PingFrame{}))
		})
	})
	...
	Context("Handshake data", func() {
		...
		It("adds a PING", func() {
			...
			Expect(q.HasData(protocol.EncryptionHandshake)).To(BeTrue())
			Expect(q.GetFrame(protocol.EncryptionHandshake, protocol.MaxByteCount, protocol.Version1)).To(Equal(&wire.PingFrame{}))
		})
	})
	...
	Context("Application data", func() {
		...
		It("adds a PING", func() {
			...
			Expect(q.HasData(protocol.Encryption1RTT)).To(BeTrue())
			Expect(q.GetFrame(protocol.Encryption1RTT, protocol.MaxByteCount, protocol.Version1)).To(Equal(&wire.PingFrame{}))
		})
	})
})
