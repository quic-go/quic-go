...
// Fuzz fuzzes the QUIC frame parser.
func Fuzz(data []byte) int {
	if len(data) < PrefixLen {
		return 0
	}
	encLevel := protocol.EncryptionLevel(data[0])
	...
	for len(data) > 0 {
		...
		checkFrameInvariants(frame)

		startLen := len(b)
		parsedLen := initialLen - len(data)
		b, err = frame.Append(b, version)
		if err != nil {
			panic(fmt.Sprintf("error writing frame %#v: %s", frame, err))
		}
		frameLen := protocol.ByteCount(len(b) - startLen)
		if frame.Length(version) != frameLen {
			panic(fmt.Sprintf("inconsistent frame length for %#v: expected %d, got %d", frame, frameLen, frame.Length(version)))
		}
		if sf, ok := frame.(*wire.StreamFrame); ok {
			sf.PutBack()
		}
...
func checkFrameInvariants(frame wire.Frame) {
	switch f := frame.(type) {
